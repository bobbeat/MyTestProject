...
#define WM_REACH_DESTINATION_NOTIFY	WM_USER + 12		/**< 模拟导航、导航到达目的地的消息 */
    ...
    /*
     * WM_TMCUPDATE:
     * wParam:GNET_TASK:	GDNET_TMC_TASK_LOGON	10001 实时交通登入
     *					GDNET_TMC_TASK_REFLUSH	10003 实时交通刷新
     * lParam: GSTATUS错误码对应表如下: 错误码  说明    错误级别
     * lParam: 0     操作成功  无
     * lParam: 1     网络错误 出错
     * lParam: 2     未知错误 出错
     * lParam: 3     网络超时 出错
     * lParam: 4     用户中断 无
     * lParam: 5     操作成功，但无数据  无
     * lParam: 10002	协议版本号错误  	出错
     * lParam: 10003	非法请求  	出错
     * lParam: 10004	厂商ID过期 	出错
     * lParam: 10005	用户ID过期 	出错
     * lParam: 10006	厂商密码错误   	出错
     * lParam: 10007	未登陆 	出错
     * lParam: 10008	导入数据错误 	出错
     * lParam: 10009	内存分配错误 	出错
     * lParam: 10010	数据不是同一批次 	出错
     * lParam: 10011	数据报文为空 	出错
     * lParam: 10012	未知错误 	出错
     * lParam: 10014	导入数据错误 	出错
     * lParam: 10015	导入数据错误 	出错
     * lParam: 10016	导入数据错误 	出错
     * lParam: 10017	非法验证码 	警告	GNaviNet内部会重新登录
     * lParam: 10018	验证码过期 	警告	GNaviNet内部会重新登录
     * lParam: 10019	请求参数非法 	出错
     * lParam: 10020	表示请求时间间隔太短,无更新数据 	警告
     * lParam: 10021	该地区无交通信息服务 	警告
     * lParam: 10022	状态码错误 	出错
     * lParam: 10023	校验结果码错误 	出错
     * lParam: 10024	解压方式未指定 	出错
     * lParam: 10025	服务端不支持当前城市 	警告
     * 由HMI层接受消息后对TMC进行更新
     */
#define WM_TMCUPDATE	WM_USER + 13			/* 更新TMC */
    ...
    /*
     * WM_NETMAP_TASK:
     * wParam: GNET_TASK:   GDNET_NETMAP_TASK_LOAD      80020  矢量数据下载
     *                      GDNET_NETMAP_TASK_TRAFFIC   80030  本网融合实时交通基础数据下载
     *                      GDNET_NETMAP_TASK_BLOCK	   80040  本网融合街区数据下载
     * lParam: GNET_STATUS: GDNET_ERR_OK                0      操作成功
     *                      GDNET_ERR_NET_FAIL          1      网络错误
     *                      GDNET_ERR_UNKNOWN           2      未知错误
     *                      GDNET_ERR_NET_TIMEOUT       3      网络超时
     *                      GDNET_ERR_NET_STOP          4      用户/网络中断
     *                      GDNET_ERR_ZERO_OK           5      操作成功，但无数据
     *
...
#define WM_TIR		WM_USER + 18			/* 事件上报信息消息 */
    ...
    /*
     * WM_REACH_JOURNEYPOINT_NOTIFY
     * wParam: 1-5 分别代表中途点1-5
     * lParam: GSTATUS错误码(GD_ERR_OK, GD_ERR_OP_CANCELED, ...)
     */
#define WM_REACH_JOURNEYPOINT_NOTIFY	WM_USER + 19		/**< 导航达到中途点消息 */
    
    /*
     * WM_ANIMATED_END:
     * 在地图操作动画结束的时候想HMI层发送该消息
     * wParam:
     * lParam: GSTATUS错误码(GD_ERR_OK, GD_ERR_OP_CANCELED, ...)
     */
#define WM_ANIMATED_END	WM_USER + 20		/**< 动画效果结束消息 */
     ...
     **********************************************************************
     ...
     该函数如果返回失败，一定不能执行后续的绘图工作，所以请一定判断该函数的
     返回值
     \since 6.0
     ...
     **********************************************************************
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_CheckResource(Gchar *pszAppPath, Gbool *pbOK);
    
    /**
     **********************************************************************
     \brief 检查指定行政区域的数据完整性
     \details 该函数用于检查指定行政区域的数据完整性
     \param[in]	pszDataPath 数据路径
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_CheckAdareaData(Gchar *pszDataPath, GADMINCODE *pstAdCode, Gbool *pbOK);
     ...
     **********************************************************************
     ...
     \retval	GD_ERR_FAILED 失败
     \remarks
     GDBL_Startup须保证调用GDBL_UninitData反初始化
     \since 7.0
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_InitData(Gchar *pzDataPath);
     ...
     **********************************************************************
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_UninitData();
     ...
     **********************************************************************
     \brief 获取色盘版本比较状态
     \details 该函数用于比较色盘版本与当前引擎匹配色盘版本
     \param[in] pczPath 色盘文件路径
     \param[out] pCompareState 比较结果
     \retval	GD_ERR_OK	成功
     \retval	GD_ERR_FAILED	失败
     \retval	GD_ERR_INVALID_PARAM	参数无效
     \since 7.0
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_GetPaletteVersionCompare(
                                                        Gchar *pczPath,
                                                        GPALETTEVERSION *pCompareState
                                                        );
     ...
     **********************************************************************
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_DeleteCityDB(GADMINCODE* pstAdcode, enumDBTYPE eDbType);
     ...
     **********************************************************************
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_IsAllModulesBusy(Gbool* bBusy);
     ...
     **********************************************************************
     ...
     G_IMAGE_TYPE_TOUR       图片ID                  无效
     G_IMAGE_TYPE_2DCAR      无效                    车标索引（0～4）共5张
     G_IMAGE_TYPE_3DCAR      无效                    车标索引（0～4）共5张
     G_IMAGE_TYPE_LANE       车道ID
     G_IMAGE_TYPE_EVENT     交通事件ID               无效
     - 2、6.0原有获取图片接口删除，统一使用该接口。
     - 3、内部不进行车道备份，获取出去HMI自行缓存、否则有可能因为内存原因被释放。
     - 4、获取车道的时候，如果nFlag等于0/2 则不判断nImageID，直接给出边框。
     - 5、引导状态的行车线有推荐及可行的状态，如果需要获取车位所在路段的行车线,调用GDMID_GetCarLanes该接口获取ID。
     - 6、对于行车线的nFlag定义：0机动点行车线左边框
     2机动点行车线右边框
     4机动点行车线
     \since 7.0
     \see
    ...
    /**
     **********************************************************************
     \brief 释放GDBL_GetImage获取的图片资源
     \details 该函数用于释放GDBL_GetImage获取的图片资源
     \param[in] pImageParam 图片类型、ID、标识
     \param[in] pBitmap 图片
     \retval	GD_ERR_OK	成功
     \retval	GD_ERR_INVALID_PARAM	参数无效
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_UnLoadImage (
                                            GIMAGEPARAM *pImageParam,
                                            GBITMAP  *pBitmap
                                            );
     ...
     **********************************************************************
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_GetImagePixs(enumGMAPIMAGETYPE euType, GIMAGEPIX **ppOutImagePixs, Gint32 *pnNum);
     ...
     **********************************************************************
     ...
     若调用了该接口，在切换横竖屏时需进行如下步骤：
     1、如果为动态效果则设置为非动态效果；
     2、调用GDBL_RecoveryView恢复；
     3、切换横竖屏并刷一下图，但无需显示该刷图结果；
     4、GDBL_ViewToOverView切换为概览；
     5、如果之前为动态效果则设置回动态效果。
     \since 7.0
     \see
     **********************************************************************/
     ...
     **********************************************************************
     \brief 计算线段角度
     \details 该函数用于计算线段角度
     \param[in]  pGeoCoordFrom 起点坐标
     \param[in]  pGeoCoordTo 终点坐标
     \param[in/out]  pAngle 角度值
     \retval	GD_ERR_OK 成功
     \retval	GD_ERR_FAILED 失败
     \remarks
     \since 7.0
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_CalcAngle (
                                          GPOINT *pGeoCoordFrom,
                                          GPOINT *pGeoCoordTo,
                                          Gfloat32 *pAngle
                                          );
     ...
     **********************************************************************
     \brief 获取对应道路id的道路属性
     \details 该函数用于获取对应道路id的道路属性
     \param[in]  pstRoadID       道路id
     \param[in]  nRoadDir        道路走向
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_GetRoadAttr(GOBJECTID *pstRoadID, Gint32 nRoadDir, GROADATTR *pstRoadAttr);
     ...
     **********************************************************************
     ...
     \param[in] pnAdCodeCnt 返回行政编码个数
     \param[out] ppstAdCode 行政编码
     \retval	GD_ERR_OK 成功
     \retval	GD_ERR_INVALID_PARAM	参数无效
     \retval	GD_ERR_NO_DATA	无相关数据
     \remarks
     pstCoordTel为经纬度坐标时，需要指定行政区域编码类型
     \since 6.0
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_GetAdminCode (
                                             GCOORDTEL *pCoordTel,
                                             Gint32 *pnAdCodeCnt,
                                             GADMINCODE **ppstAdCode
                                             );
    
    /**
     **********************************************************************
     \brief 获取指定级别的区域编码
     \details 获取行政区域编码指定级别的区域编码
     \param[in] pstAdCode 行政区域编码
     \param[in] euAdCodeLevel 指定获取级别
     \param[out] pstLevelAdCode 指定级别区域编码
     \retval	GD_ERR_OK 成功
     \retval	GD_ERR_INVALID_PARAM 参数无效
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_GetSpecLevelAdCode(GADMINCODE *pstAdCode, GADAREATYPE euAdCodeLevel, GADMINCODE *pstLevelAdCode);
     ...
     **********************************************************************
     ...
     \retval	GD_ERR_OK               成功
     \retval	GD_ERR_INVALID_PARAM    参数无效
     \retval	GD_ERR_RUNNING          要删除的引导路径还在引导
     \retval	GD_ERR_FAILED           操作失败
     \remarks
     - 1.移除操作并不做实际的删除引导路径。
     - 2.正在引导的引导路径不能移除。
     ...
     **********************************************************************
     \brief 选择引导路径
     \details 该函数用于选择一条路径作为当前路径。
     \param[in] hGuideRoute     路径句柄
     \retval	GD_ERR_OK               成功
     \retval	GD_ERR_INVALID_PARAM    参数无效
     \retval	GD_ERR_FAILED           引导初始化失败
     \remarks
     - 1.选择一条引导路径并不代表引导的就是这条路径。
     - 2.选择功能在多条路径时，选择一条后，可以高亮显示，以供查看。
     - 3.该接口不允许传空的句柄
     - 4.调用该接口后，每个行程点的规划原则会变成传入句柄的规划原则。
     - 5.调用该接口后，跟路径相关的所有视图句柄均会被释放。(包括:单路径、多路径全程概览，路径不同处对比，路口详情，路径TMC概览)
     \since 6.0
     \see
     **********************************************************************/
     ...
     **********************************************************************
     ...
     \param[in] hGuideRoute            路径句柄
     \param[in] euGuideRoutePos        获取路径经过城市的信息类型
     \param[in] euAdaLevel             获取的行政编码的级别
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_GetGuideRouteCityInfo (GHGUIDEROUTE hGuideRoute,
                                                      GGUIDEROUTETYPE euGuideRouteType,
                                                      GADALEVEL euAdaLevel,
     ...
     **********************************************************************
     \brief 结束引导
     \details 该函数用于结束驾驶引导。
     \param[in] bClearAllRoute 是否删除所有引导路径
     \retval	GD_ERR_OK           成功
     \retval	GD_ERR_RUNNING      正在模拟导航
     \remarks
     - 1.bClearAllRoute：Gfalse 停止路径引导，不删除引导路径。
     - 2.bClearAllRoute：Gtrue  结束路径引导，清空引导路径列表。
     - 3.调用该接口后，跟路径相关的所有视图句柄均会被释放。(包括:单路径、多路径全程概览，路径不同处对比，路口详情，路径TMC概览)
     \since 6.0
     \see
     **********************************************************************/
     ...
     **********************************************************************
     ...
     - 2.如果要关闭放大路口请设置参数G_MAP_SHOW_ZOOM_VIEW: Gtrue:显示放大路口 Gfalse:关闭放大路口
     \since 6.0
     \see
     **********************************************************************/
     ...
     **********************************************************************
     ...
     \retval	GD_ERR_FAILED	语音播报回调函数空
     \remarks
     \since 6.0
     \see
     **********************************************************************/
     ...
     **********************************************************************
     ...
     \retval	GD_ERR_OK       成功
     \retval	GD_ERR_NO_ROUTE	没有引导路径
     \retval	GD_ERR_NO_DATA  没有获取引导语音
     \retval	GD_ERR_FAILED	语音播报回调函数为空
     \remarks
     - 1.该函数只播报最近一次获取的引导语音
     \since 6.0
     \see
     ...
     **********************************************************************
     ...
     \retval	GD_ERR_NO_MEMORY        内存不足
     \retval	GD_ERR_INVALID_PARAM    参数错误
     \retval	GD_ERR_NO_DATA          获取不到引导数据
     \remarks
     - 1.引导道路列表内存由内部开辟，每次调用均会替换上次数据。
     \since 6.0
     \see
     **********************************************************************/
     ...
     **********************************************************************
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_AddAvoidMainRoad(
                                                GHGUIDEROUTE hGuideRoute,
                                                GDETOUROPTION eOption,
                                                Gint32 nNodeNo
                                                );
     ...
     **********************************************************************
     ...
     \retval	GD_ERR_NO_DATA          获取不到引导数据
     \remarks
     - 1.路径统计信息内存由内部开辟，每次调用均会替换上次数据。
     - 2.如果bMultiStat为true则参数hGuideRoute无效，默认获取路径列表中所有的路径信息
     \since 6.0
     \see
     **********************************************************************/
     ...
     **********************************************************************
     ...
     \retval	GD_ERR_INVALID_PARAM    参数错误
     \retval	GD_ERR_FAILED           路线保存失败
     \retval	GD_ERR_NO_DATA          引导数据获取不到、或者起点绑定不到
     \remarks
     - 1.文件路径一定要传绝对路径。
     - 2.实际引导的时候，起点改为以当前车位为起点。
     - 3.如果车位距中途点不足20m，中间层会自动舍弃中途点
     \since 6.0
     \see
     **********************************************************************/
     ...
     **********************************************************************
     \brief  路径加载
     \details 该函数用于加载一条事先存储的引导路径进行引导。
     \param[in]  szFileName	  需要加载的文件名
     \param[out] nRule	      各行程点的规划原则
     \retval	GD_ERR_OK                   成功
     \retval	GD_ERR_VER_INCOMPATIBLE     版本不匹配
     \retval	GD_ERR_INVALID_PARAM        参数错误
     \retval	GD_ERR_FAILED               路径加载失败
     \remarks
     - 1.文件名是绝对路径。
     - 2.HMI调用该接口后 还需要调用路径演算接口进行进一步算路。
     - 3.HMI调用路径演算可以自由决定算单路径、多路径、或者只返回路径句柄而不进行引导。
     - 4.对于单路径由于7.x支持每个到达点都有不同的规划原则，因此会以数组的形式给出
     - 5.对于多路径只需要判断数组最后一个元素的规划原则即可（即终点的规划原则）。
     \since 6.0
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_LoadGuideRoute(
                                              Gchar *szFileName,
                                              Gint32 nRule[GJOURNEY_MAX]
                                              );
     ...
     **********************************************************************
     ...
     \retval	GD_ERR_OK           成功
     \retval	GD_ERR_NO_ROUTE     没有引导路径
     \retval	GD_ERR_FAILED       处于移图状态不返回数据
     \remarks
     - 1.处于移图状态下不返回任何状态
     \since 6.0
     \see
     ...
     **********************************************************************
     ...
     \retval	GD_ERR_FAILED           暂无具体错误码
     \remarks
     - 1.用于获取最近一个被跳过的中途点。该接口只有在中途点被绕过后首次调用才返回正确值
     ...
     **********************************************************************
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_SetTrackLineInfo(GTRACKLINEINFO *pTrackLineInfo, Gint32 nNum, GRECT *pstRect);
     ...
     **********************************************************************
     ...
     \see
     **********************************************************************/
    GDBL_API_CALL GSTATUS GDBL_GetDetourRoadCityInfo(Gchar *szFileName, GDETOURROADCITYINFO **ppCityInfos, Gint32 *pnNumberOfCity);
