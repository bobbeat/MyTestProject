...
@implementation MWDialectDownloadTask

@synthesize version,folderName,dialectRoleID,dialectPlaySpeed,dialectHasRecord,beNeedUpdate,dialectFontType,md5;
...
- (void)encodeWithCoder:(NSCoder *)encoder
{
	...
	[encoder encodeObject:folderName forKey:@"tasklist8key"];
	[encoder encodeInt64:dialectRoleID forKey:@"tasklist9key"];
	[encoder encodeInt64:dialectPlaySpeed forKey:@"tasklist10key"];
	[encoder encodeBool:dialectHasRecord forKey:@"tasklist11key"];
	[encoder encodeBool:beNeedUpdate forKey:@"tasklist12key"];
    [encoder encodeInt:dialectFontType forKey:@"tasklist13key"];
    [encoder encodeObject:md5 forKey:@"tasklist14key"];
}
...
- (id)initWithCoder:(NSCoder *) decoder
{
	if (self = [super init])
	{
		...
		self.folderName = [decoder decodeObjectForKey:@"tasklist8key"];
		self.dialectRoleID = [decoder decodeInt64ForKey:@"tasklist9key"];
		self.dialectPlaySpeed = [decoder decodeInt64ForKey:@"tasklist10key"];
		self.dialectHasRecord = [decoder decodeBoolForKey:@"tasklist11key"];
		self.beNeedUpdate = [decoder decodeBoolForKey:@"tasklist12key"];
        self.dialectFontType = [decoder decodeIntForKey:@"tasklist13key"];
        self.md5 = [decoder decodeObjectForKey:@"tasklist14key"];
	}
...
- (void)createFilepath
{
	if (![[NSFileManager defaultManager] fileExistsAtPath:Dialect_path]) //要是文件目录不存在，创建目录
	{
		[[NSFileManager defaultManager] createDirectoryAtPath:Dialect_path withIntermediateDirectories:NO attributes:nil error:nil];
	}
	
	// 公司内部协议，未完成下载的文件统一未.tmp文件
	filePath = [[NSString alloc] initWithString:[Dialect_path stringByAppendingFormat:@"%@.tmp",filename]];
}

// 执行下载任务
- (void)run
{
    NSLog(@"DialectUrl=%@",url);
	//判断url是否为空，如果为空，提示。
	if (url == nil)
	{
...
- (void)connectionDidFinishLoading:(NSURLConnection *)aconnection
{
	...
	NSString* destinctionPath = [Dialect_path stringByAppendingFormat:@"%@",filename];
	...
	if ([[NSFileManager defaultManager] fileExistsAtPath:Dialect_path])
	{
        ...
        //md5校验
        NSString *temp_path = [NSString stringWithFormat:@"%@%@",Dialect_path,[self getFilename]];
...
- (void)erase
{
    ...
    fileName = [[NSString alloc] initWithString:[Dialect_path stringByAppendingFormat:@"%@.tmp",temp_path]];
	// 删除临时文件并初始化下载数据
    ...
    NSString *dialectFileName = [[NSString alloc] initWithString:[Dialect_path stringByAppendingFormat:@"%@",folderName]];
    ...
    if ([[MWPreference sharedInstance] getValue:PREF_IS_LZLDIALECT] == self.taskId) {
...
{
    NSString *gdtmpPath = [NSString stringWithFormat:@"%@%@",Dialect_path,[self getFilename]];
...
- (void)ZipArchive
{
	...
	if ([za UnzipOpenFile:destinctionPath])
	{
		...
		do
        {
			...
			int zaResult = [za UnzipFileTo:Dialect_path overWrite:YES retValue:&ret filesize:disksize];
			if (zaResult!=2)
			{
				if (zaResult==1)
                {
