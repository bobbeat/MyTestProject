...
/** 初始化控件
 ...
 @param origin 控件的坐标
 @param isCenter 设置origin定位的位置，YES时在中心点
 @param initParam 初始化的参数
 @param relative NO:初始化时识别UI中心点相对于home键不变；YES:识别UI左上角相对于home键不变。一般情况下建议使用NO。
 @return
 */
- (id)initWithOrigin:(CGPoint)origin atCenter:(BOOL)isCenter initParam:(NSString *)initParam relative:(BOOL)relative;

/** 旋转控件
 
 @param toInterfaceOrientation 视图控制器旋转的方向
 */
- (void) rotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation;

/** 自定义位置旋转控件
 用于旋转后重新设定控件的位置，如果不用重设旋转后的位置，可以使用上面的方法。
 
 @param toInterfaceOrientation 视图控制器旋转的方向
 @param point 定位识别控件左上角，具体坐标任何时刻均以设备左上角为原点。即相对于home键不变。
 @param isCenter pointd定位的位置，YES时在中心点
 */
- (void) rotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation toPoint:(CGPoint)point atCenter:(BOOL)isCenter;
