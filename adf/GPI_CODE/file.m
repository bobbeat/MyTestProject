...
//	convertUTF32to8(fn, szFileName);
//    convertUTF32to8(md, szMode);
...
{
	Gchar* ret;
	Gchar *szTmp = (Gchar *)malloc(size * sizeof(Gchar));
	ret = (Gchar*)fgets((char*)buffer, size, (FILE*)fp);
	Gmbstowcs(szTmp, (Guint32)size, (Guint8 *)buffer);
	Gstrcpy(buffer, szTmp);
    free(szTmp);
	return ret;
}
...
{
//    NSString *dirPath = Cstring32ToNSString(szDir);
//    NSFileManager *manager = [NSFileManager defaultManager];
//    NSArray *array = [manager contentsOfDirectoryAtPath:dirPath error:nil];
//    FILEINFO fi = {0};
//    NSString *match = Cstring32ToNSString(szPattern);
...
//        GcharMemcpy(fi.szFileName, NSStringToCstring32([array objectAtIndex:i]), GMAX_PATH);
//        lpFindFileCB(&fi);
//    }
...
Guint32 GPI_RenameFile(const Gchar *szOldFileName, const Gchar *szNewFileName)
{
	...
	GstrcpyAG(oldpath, (Gchar *)szOldFileName);
	GstrcpyAG(newpath, (Gchar *)szNewFileName);
...
Guint32 GPI_DeleteFile(const Gchar *szFileName)
{
	int ret = 0;
	Gint8 path[GMAX_PATH] = {0};
	GstrcpyAG(path, (Gchar *)szFileName);
...
{
    Gint8 path[GMAX_PATH] = {0};
	GstrcpyAG(path, (Gchar *)szDirectory);
    NSString *imageDir = [NSString stringWithFormat:@"%s",path];
