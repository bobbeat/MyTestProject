...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite 
{
	...
	do{
		...
		if( isDirectory )
		{
			...
			NSString *stringt = @"Documents";
			NSRange range = [fullPath rangeOfString:stringt];
			NSString *strdata = [fullPath substringFromIndex:range.location+10];//去除前面的无关信息
//			NSLog(@"%@",strdata);
		}
		else
		{
...
- (int)getRetValue
{
	return unzGoToFirstFile( _unzFile );
}


- (int)UnzipFileTo:(NSString *)path overWrite:(BOOL)overwrite retValue:(int *)ret filesize:(long long)fsize
{
	int result=0;
	unsigned char		buffer[4096] = {0};
	NSFileManager* fman = [NSFileManager defaultManager];
	...
	*ret = unzGetCurrentFileInfo(_unzFile, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
	...
	if (fsize<fileInfo.uncompressed_size) 
	{
		result=1;
		return result;
	}
	...
	char* filename = (char*) malloc( fileInfo.size_filename +1 );
	NSLog(@"Bfilename==%s",filename);
	unzGetCurrentFileInfo(_unzFile, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
	filename[fileInfo.size_filename] = '\0';
	NSLog(@"Efilename==%s",filename);
...
-(NSString *) UnzipFileToPath:(NSString*) path overWrite:(BOOL) overwrite
{
	NSString *success;
	int ret = unzGoToFirstFile( _unzFile );
	unsigned char		buffer[4096] = {0};
	...
	do{
		...
		if( isDirectory )
		{
			...
			NSRange range = [fullPath rangeOfString:stringt];
			//NSString *strdata = [fullPath substringFromIndex:range.location+10];//去除前面的无关信息
			success = [fullPath substringFromIndex:range.location+10];//去除前面的无关信息
//			NSLog(@"%@",success);
		}
		else
		{
