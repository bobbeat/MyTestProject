...
@interface MainViewController ()<RouteCalDelegate>
...
- (void)Action_GoToCCP
{
    ...
    NSString *str = [containerURL.path stringByAppendingString:@"/gddownload/c0508.zip"];
    
    NSFileManager *manager = [NSFileManager defaultManager];
    
    NSArray *array = [manager contentsOfDirectoryAtPath:containerURL.path error:nil];
    NSLog(@"%@",array);
    for (NSString *temp in array)
    {
        NSLog(@"%@",[[temp class] description]);
        NSString *direct = [containerURL.path stringByAppendingPathComponent:temp];
        NSArray *array_temp = [manager contentsOfDirectoryAtPath:direct error:nil];
        NSLog(@"%@",array_temp);
    }
    
    BOOL sign = [[NSFileManager defaultManager] fileExistsAtPath:str];
    if (sign)
    {
        NSData *data = [NSData dataWithContentsOfFile:str];
        NSLog(@"%ld",(unsigned long)[data length]);
        [manager removeItemAtPath:str error:nil];
        UIAlertView *view = [[UIAlertView alloc] initWithTitle:@"title" message:[NSString stringWithFormat:@"存在数据c0508.zip，大小=%ld,且删除",(unsigned long)[data length]] delegate:nil cancelButtonTitle:@"ok" otherButtonTitles: nil];
        [view show];
        [view release];
    }
    else
    {
        UIAlertView *view = [[UIAlertView alloc] initWithTitle:@"title" message:[NSString stringWithFormat:@"不存在数据c0508.zip"] delegate:nil cancelButtonTitle:@"ok" otherButtonTitles: nil];
        [view show];
        [view release];
    }
    
    NSArray *array34 = [manager contentsOfDirectoryAtPath:containerURL.path error:nil];
    return;
