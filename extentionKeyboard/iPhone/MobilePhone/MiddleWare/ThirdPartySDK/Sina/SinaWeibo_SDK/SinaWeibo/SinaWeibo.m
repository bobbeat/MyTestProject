...
- (void)logIn:(UINavigationController *)controller
{
    if ([self isAuthValid])
    {
        if ([delegate respondsToSelector:@selector(sinaweiboAreadyLogIn:)])
        {
            [delegate sinaweiboAreadyLogIn:self];
        }
    }
    else
    {
...
//        }
        ...
        if (!ssoLoggingIn)
        {
            ...
            [[SinaWeiboAuthorizeView alloc] initWithAuthParams:params
                                                      delegate:self];
            [authorizeView show];
            [controller pushViewController:authorizeView animated:YES];
...
{
    NSString *urlString = [url absoluteString];
    if ([urlString hasPrefix:kAppCallbackScheme])
    {
...
- (void)didReceiveWeiboResponse:(WBBaseResponse *)response
{
    if ([response isKindOfClass:WBAuthorizeResponse.class])
    {
        if (response.statusCode == WeiboSDKResponseStatusCodeUnknown) {
            if (delegate && [delegate respondsToSelector:@selector(sinaweiboLogInDidCancel:)])
            {
                [delegate sinaweiboLogInDidCancel:self];
            }
        }
        ...
        {
            if (delegate && [delegate respondsToSelector:@selector(sinaweibo:logInDidFailWithError:)])
            {
                NSString *error_description = @"Invalid sso params";
                NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:
                                          error_description, NSLocalizedDescriptionKey, nil];
            ...
            NSString *expires_in = [response.userInfo objectForKey:@"expires_in"];
            NSString *remind_in = [response.userInfo objectForKey:@"remind_in"];
            NSString *uid = [(WBAuthorizeResponse *)response userID];
            NSString *refresh_token = [response.userInfo objectForKey:@"refresh_token"];
