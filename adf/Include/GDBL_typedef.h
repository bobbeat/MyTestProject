...
typedef struct  tagGIMAGEPARAM {
	...
	Gint32		nFlag;					/**< 其他标识，对于转向图标传0代表小图标，传非0代表大图标 */
} GIMAGEPARAM;
...
typedef enum tagGTMCSHOWOPTION {
	GMAP_TMC_SHOW_CITY  = 0,	/**< 城市TMC */
	GMAP_TMC_SHOW_ROUTE = 1		/**< 路径TMC */
} GTMCSHOWOPTION;
...
{
	GLOCDATATYPE euDataType;		/**< 本结构体类型 */
	Guint32	nTimestamp;				/**< 时间戳 */
	Gint32	nDeviceAttitude;		/**< 1竖放头向上，2横放左向上，3竖放头向下，4横放右向上 */
...
typedef struct tagGGYRODATA{
	GLOCDATATYPE euDataType;/**< 本结构体类型 */
	Guint32		nTimestamp;	/**< 时间戳 */
	Guint32		nCount;		/**< 陀螺数据个数 */
	Guint32		nInterval; 	/**< 每个数据时间间隔 */
	Gint32		Value[10];	/**< 陀螺数值 */
} GGYRODATA;
...
typedef enum tagGADAREATYPE{
	ADAREA_TYPE_PROVINCE = 0,	/**< 省类型 */
	ADAREA_TYPE_CITY,			/**< 市类型 */
	ADAREA_TYPE_TOWN	,		/**< 区类型 */
	ADAREA_TYPE_CHINA,			/**< 全国类型 */
}GADAREATYPE;
...
typedef struct tagGADAREA{
	...
	Gint32	lNumberOfSubAdarea;					/**< 下级行政区域个数 */
...
typedef enum tagGGUIDEROUTETYPE {
	GGUIDEROUTE_TYPE_CARPOS = 0,	/**< 获取从车位到目的城市路径经过的城市信息 */
	GGUIDEROUTE_TYPE_ALL	= 1		/**< 获取整条路径经过的城市信息 */
} GGUIDEROUTETYPE;
...
typedef enum tagGPARAM {
	...
	G_MAP_TMC_SHOW_OPTION		= 0x0601,		/**< 地图TMC显示规则：全市或者只显示路径线上的 */
	...
	G_DEMO_SPEED				= 0x0801,		/**< 模拟导航速度 7.0引擎支持40～200(Km/h) 放大路口会强制把速速设置为40km/h */
	...
	/* safe */
	G_SAFETY_INFORMATION		= 0x0f00,		/**< 安全驾驶开关 */
	G_SAFE_DANGERWARNPART1		= 0x0f01,		/**< 设置母库安全驾驶高危险警示级别类型1：参数默认值需要根据参数取值范围危险警示类型的进行移位设置（默认值可由取值范围内的值进行“与”运算得出的整形） */
	G_SAFE_DANGERWARNPART2		= 0x0f02,		/**< 设置母库安全驾驶高危险警示级别类型1：参数默认值需要根据参数取值范围危险警示类型的进行移位设置（默认值可由取值范围内的值进行“与”运算得出的整形） */
	G_SAFE_SPEEDLIMITSPEAK		= 0x0f03,		/**< 限速路牌播报提供三个选择项：全部播报、 仅超速播报， 默认仅超速播报 */
	G_SAFE_CAMERASPEAK			= 0x0f04,		/**< 摄像头播报提供三个选择项：全部播报、仅罚单播报；默认仅罚单播报 */
	G_SAFE_DANGERWARNSPEAK		= 0x0f05,		/**< 危险警示播报提供三个选择项：全部播报、仅高危播报、不播报； 默认仅高危播报 */
	G_SAFE_SPEAK_DIS_CAMERA		= 0x0f09,		/**< 摄像头播报距离 */
	G_SAFE_SPEAK_DIS_LIMIT		= 0x0f0a,		/**< 限速播报距离 */
	G_SAFE_SPEAK_DIS_WARN		= 0x0f0b,		/**< 危险区域播报距离 */
	G_SPEAK_ACCURACY_SAFETY		= 0x0f0c,		/**< 安全驾驶精确播报开关 */
    ...
    G_SHOW_TMCSTREAM			= 0x1113,       /**< 显示实时交通流 */
    G_SHOW_TMCEVENT				= 0x1114,       /**< 显示实时交通事件 */
    G_SHOW_TMCSTREAMEVENT       = 0x1115,       /**< 显示实时交通流事件 */
 ...
 *
 */
typedef enum tagGSAFETYPE
{
	GSAFE_SAFETYPE_LIMITSPEED = 0,		/**< 限速标志 */
	GSAFE_SAFETYPE_CAMERA = 1,			/**< 摄像头  */
	GSAFE_SAFETYPE_DANGERWARN = 2,		/**< 危险警示标志  */
}SAFETYPE;
...
typedef enum tagGSAFEDANGERWARNPART1
{
	...
	GSAF_SLOWDOWNGIVEWAY        	= 0x03000000, /**< 减速让行 */
	GSAF_CARRETURNGIVEWAY       	= 0x04000000, /**< 会车让行 */
	GSAF_RIGHTNARROW            	= 0x08000000, /**< 右侧变窄 */
	GSAF_LEFTNARROW             	= 0x10000000, /**< 左侧变窄 */
	GSAF_NARROWBRIDGE           	= 0x20000000, /**< 窄桥 */
	GSAF_LEFTRIGHTAROUND	        = 0x40000000, /**< 左右绕行 */
...
typedef enum tagGSAFEDANGERWARNPART2
{
	...
	GSAF_CONSTRUCTION           	= 0x03000000, /**< 施工 */
...
{
	GSAFE_CAMERASPEAK_ALL = 0,		/**< 全部播报 */
	GSAFE_CAMERASPEAK_TICKET = 1,	/**< 仅罚单播报  */
	GSAFE_CAMERASPEAK_CLOSE = 2,	/**< 不播报  */
}GSAFECAMERASPEAK;
...
{
	GSAFE_DANGERWARNSPEAK_ALL = 0,		/**< 全部播报 */
	GSAFE_DANGERWARNSPEAK_HIGHRISK = 1,	/**< 仅高危播报  */
	GSAFE_DANGERWARNSPEAK_CLOSE = 2,	/**< 不播报  */
