...
//#define AppDelegate 	(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)?((AppDelegate_iPhone*)[[UIApplication sharedApplication] delegate]):((AppDelegate_iPad*)[[UIApplication sharedApplication] delegate])
//#define MainController [AppDelegate mainViewController]
//#define MainView [MainController view]
#define MainView ((UIWindow *)[[UIApplication sharedApplication].windows objectAtIndex:0]).rootViewController.view//    [[[UIApplication sharedApplication] delegate].navigationController.view];
...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation {
	...
	if (OrientationSwitch == 1) { //横竖屏开关－关
        if (Interface_Flag == 0) {
           // [self set_HV:0];
        }
	...
	else if(interfaceOrientation == 1 || interfaceOrientation == UIDeviceOrientationPortraitUpsideDown)
	{
...
{
    [POICommon showAutoHideAlertView:text showTime:time];
}
